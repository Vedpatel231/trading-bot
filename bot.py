...
CRYPTO_TIMEFRAME    = "1m"       # entry signal timeframe
CRYPTO_HTF          = "5m"       # higher timeframe trend filter
FAST_EMA            = 5
SLOW_EMA            = 50
RSI_PERIOD          = 14
RSI_OVERBOUGHT      = 70
...
RISK_PER_TRADE      = 0.02
CRYPTO_INTERVAL     = 60          # check every 1 minute
...
def fetch_crypto(symbol, timeframe, limit=200):
...
def get_htf_trend(symbol):
    try:
        df   = fetch_crypto(symbol, CRYPTO_HTF, limit=100)
    ...
    while True:
        try:
            ...
            for symbol in CRYPTO_SYMBOLS:
                coin = symbol.split("/")[0]
                try:
                    df_now = fetch_crypto(symbol, CRYPTO_TIMEFRAME, limit=5)
                    check_crypto_exits(symbol, df_now["close"].iloc[-1])
                    htf    = get_htf_trend(symbol)
                    df     = fetch_crypto(symbol, CRYPTO_TIMEFRAME, limit=200)
                    df     = add_indicators(df)
                    signal, price, rsi = get_signal(df)
                    status = "IN TRADE" if crypto_paper[symbol]["in_trade"] else "watching"
                    print(f"  {coin:<4} | {signal:<4} | 5m: {htf:<7} | "
                          f"RSI: {rsi:>5.1f} | ${price:>10,.2f} | {status}")
                    if signal == "BUY":
                        if htf == "UP": crypto_buy(symbol, price)
                        else: print(f"        BUY blocked — 5m trend: {htf}")
                    elif signal == "SELL":
        ...
        try:
            ...
            if not clock.is_open:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] "
                      f"Market closed — opens in ~{mins} min. Sleeping 15 min...")
                time.sleep(60 * 15)
                continue
            ...
            for symbol in STOCK_SYMBOLS:
                try:
                    # Daily bars — reliable, always enough history for RSI
                    bars = alpaca.get_bars(symbol, TimeFrame.Day, limit=200).df
                    bars["ema_fast"] = ta.trend.ema_indicator(bars["close"], window=STOCK_FAST_EMA)
                    bars["ema_slow"] = ta.trend.ema_indicator(bars["close"], window=STOCK_SLOW_EMA)
                    ...
                    if pd.isna(rsi):
                        print(f"  {symbol:<4} | not enough data yet")
                        continue

                    htf = "UP" if last["ema_fast"] > last["ema_slow"] else "DOWN"
                    ...
                    if p["in_trade"]:
                        if price > p["highest_price"]: p["highest_price"] = price
                        if price <= p["highest_price"] * (1 - STOP_LOSS_PCT):
                            stock_sell(symbol, price, alpaca, reason="Trailing stop")
                        elif price >= p["entry_price"] * (1 + TAKE_PROFIT_PCT):
                            stock_sell(symbol, price, alpaca, reason="Take profit")
                    ...
                    if signal == "BUY":
                        ...
                        else: print(f"        BUY blocked — trend: {htf}")
                    elif signal == "SELL":
...
if __name__ == "__main__":
    # 1. Health server first
    threading.Thread(target=start_health_server, daemon=True).start()
    print(f"Health server starting on port {os.getenv('PORT', 8080)}")
    time.sleep(3)

    # 2. Crypto bot
    threading.Thread(target=run_crypto, daemon=True).start()

    # 3. Stocks bot (5 sec after crypto)
    time.sleep(5)
